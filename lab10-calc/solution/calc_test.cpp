...
// These macros should be macros to preserve file and line information.
#ifdef TEST_COMPLEX_EXPRESSIONS
#define REQUIRE_ERR(got, expected) REQUIRE((got) == (expected))
#define CHECK_POS(got, expected) CHECK((got) == (expected))
#else
#define REQUIRE_ERR(got, expected) REQUIRE((got) != 0)
#define CHECK_POS(got, expected) void()
#endif
...
TEST_CASE("Extra input") {
    calc_result res;
    // The `12` prefix is a valid expression, but `12x` is not a valid prefix of
    // any expression.
    REQUIRE_ERR(calc_evaluate("12x34", &res), CALC_ERROR_EXTRA_INPUT);
    CHECK_POS(res.error_position, 2);
}

TEST_CASE("Unexpected char end of line") {
    calc_result res;
    // The `12*` prefix can be continued to a valid expression, but `\0` is not
    // a valid continuation.
    SUBCASE("No spaces") {
        REQUIRE_ERR(calc_evaluate("12*", &res), CALC_ERROR_UNEXPECTED_CHAR);
        CHECK_POS(res.error_position, 3);
    }
    SUBCASE("With spaces") {
        REQUIRE_ERR(calc_evaluate("12* ", &res), CALC_ERROR_UNEXPECTED_CHAR);
        CHECK_POS(res.error_position, 4);
    }
}

TEST_CASE("Unexpected char middle") {
    calc_result res;
    // The `12*` prefix can be continued to a valid expression, but `12**`
    // cannot.
    REQUIRE_ERR(calc_evaluate("12**34", &res), CALC_ERROR_UNEXPECTED_CHAR);
    CHECK_POS(res.error_position, 3);
}

TEST_CASE("Bad number") {
    calc_result res;
    // `-` tells the parser to expect a number, but no prefix of `-wtf` can be
    // parsed as a number.
    REQUIRE_ERR(calc_evaluate("12*-wtf", &res), CALC_ERROR_BAD_NUMBER);
    CHECK_POS(res.error_position, 3);
}
...
TEST_CASE("Call with nullptr functions and no functions") {
    calc_result res;
    REQUIRE(calc_evaluate("3*4", &res, nullptr) == 0);
    CHECK(res.value == doctest::Approx(12));
}

TEST_CASE("Call with nullptr result success") {
    REQUIRE(calc_evaluate("3*4", nullptr) == 0);
}

TEST_CASE("Call with nullptr result error") {
    REQUIRE_ERR(calc_evaluate("3x4", nullptr), CALC_ERROR_EXTRA_INPUT);
}

#ifdef TEST_FUNCTIONS
static_assert(CALC_MAX_ARITY >= 3);

TEST_CASE("Expected open paren") {
    calc_result res;
    // The `12*sqrt` prefix seems like a beginning of `12*sqrt(10)`, but a open
    // paren should follow.
    SUBCASE("No spaces") {
        REQUIRE_ERR(
            calc_evaluate("12*sqrt+", &res), CALC_ERROR_EXPECTED_OPEN_PAREN
        );
        CHECK_POS(res.error_position, 7);
    }
    SUBCASE("With spaces") {
        REQUIRE_ERR(
            calc_evaluate("12*sqrt +", &res), CALC_ERROR_EXPECTED_OPEN_PAREN
        );
        CHECK_POS(res.error_position, 8);
    }
}

TEST_CASE("Call with nullptr functions and standard functions") {
    calc_result res;
...
TEST_CASE("Call with custom functions") {
    ...
    SUBCASE("Call predefined function which should be missing") {
        calc_result res;
        REQUIRE_ERR(
            calc_evaluate("2+pow(3,4)", &res, funcs),
            CALC_ERROR_UNKNOWN_FUNCTION
        );
        CHECK_POS(res.error_position, 5);
    }
}
...
TEST_CASE("Complicated expression") {
    calc_result res;
    REQUIRE(
        calc_evaluate("2+3*(4+5*-8/   2* 3)/3-pow ( 2*5 , 1+1 ) ", &res) == 0
    );
...
TEST_CASE("Long expression") {
    ...
    // Work around cppcheck issue with false positive 'unreadVariable'.  Cannot
    // suppress it because if 'TEST_COMPLEX_EXPRESSIONS' is not defined,
    // the suppression is "unmatched".
    (void)s;
